...
It has been proven that any distance-based method (by considering space as continuous) circumvents statistical bias associated with the Modifiable Areal Unit Problem – MAUP [@Openshaw1979; @Arbia1989] due to discretising space into separate units.
...
**In our paper, we propose to test the effectiveness of @Tidu2023's method and help the researchers to choose the appropriate method to characterize the spatial structure of quite large datasets.**
First, we show the advantages to estimate *M* on datasets with an order of magnitude of 100,000 points or less, and we show that the computation times become excessive beyond that, on a personal computer.
...
The idea of *M* is simple: it compares two proportions of neighbours of interest, a local one to a global one.
...
A 20-by-20 square grid seems visually appropriate for assessing the repositioning of the points.
The choice of this grid size is, at this step of our analysis, completely arbitrary.
...
In the *dbmss* package, data is a set of points or a distance matrix.
...
It allows parallelising the necessary simulations to save time.
...
The median execution time is equal to `r Mmb$Time[1]` milliseconds for estimating the *M* function from a set of `r format(as.numeric(par_points_nb), scientific=FALSE, big.mark=",")` points but `r Mmb$Time[2]` milliseconds for the corresponding distance matrix.
...
(ref:TestMemTime) Calculation time (a) in seconds and memory required (b) in MB to estimate *M* as a function of the size of the set of points. The bars represent the $\pm 1$ standard deviation interval.
...
The memory used by `Dtable` objects to calculate $M$ from a distance matrix is much greater: it is that of a numerical matrix, amounting to 8 bytes multiplied by the square of the number of points, i.e. 800 MB for 10,000 points only.
Since the calculation time is not reduced by this approach, its use should be reserved for non-Euclidean distances.
...
The first consequence is that **no spatial structure in each cell can be detected because the information is lost.**
The merits of any distance-based methods is undoubtedly detecting the exact geographic scale(s) at which any patterns of aggregation, dispersion or independence of points exist.
...
Both are made of 100,000 points so that groups include 250 points on average.
...
All neighbours at distances less than this threshold are placed at zero distance.
