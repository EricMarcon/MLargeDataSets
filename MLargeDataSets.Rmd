...
Let's consider the simulation of the Cases obtained by the Matérn process and cut the window into a 20-by-20 squared grid. <!--# Flo: motivation 20x20 -->
...
It is important to underline that choice of the optimal level of the grid seems today an open question as @Arbia2021 write (p.109): *'Unfortunately, the choice of the partitioning scheme is usually arbitrary and an optimal criterion to guide this choice is not available.'* 
...
The information on interactions at very short distances, i.e. within each grid cell, is lost, or, more precisely, approximated by its value at the grid scale. As a result, under the size of the grid the approximation of locations is not optimal.
...
Concerning the error generated in the *M*'s estimates results if the approximation of location is used, our results more or less support @Tidu2023's article, which mentions strong correlations between *M* values computed from exact and approximated Italian company location data.
