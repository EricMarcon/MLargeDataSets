...
abstract: >
  ...
  A precise evaluation of the computational time and memory requirements for geolocalised data is at first carried out using the *dbmss* package in R [@dbmss] by means of performance tests. 
  ...
  We give evidence that the individual location approximation generates a greater information loss at small distances.
  We recommand to use the approximation with caution for these distances.
...
This is important because it has been proved that any distance-based method (by considering space as continuous) circumvents statistical bias associated with to the Modifiable Areal Unit Problem – MAUP [@Arbia1989 ; @Openshaw1979] by discretize space into separate units.
...
## Main idea
@Marcon2010 introduced the *M* function that evaluates the dependence between geolocalised points without relying on a specific zoning of space.
...
\item a spatial concentration (attraction) of entities if the proportion of local neighbours is greater than the one observed on the entire territory,
\item a spatial dispersion (repulsion) of entities if the relative proportion of local neighbours is lesser than the one observed all over the territory,
\item independence between entities if the local distribution of neighbours does not differ from the global one.
\end{itemize}
...
Moreover, an easy-computation of *M* is possible thanks to the *dbmss* R package [@dbmss].
...
This methodology has also rapidly been applied in other sciences including biology [@Fernandez2005], geography [@Deurloo2008], ecology [@Marcon2012d] or seismology [@Nissi2013].
...
The $M$ function is based on the point process theory [@Moller2004; @Baddeley2016a].
...
In the present article, we only consider the intra-type version of *M*: we study the spatial structure of neighbouring points of the same type (called *points of interest*) as the point at the center of the disks of radius $r$.
In a mathematical terms, let us denote:
\begin{itemize}[leftmargin=*]
   \item $x^s_i$, the geolocalised position of point $i$ of the reference type $s$, at the center of the disk (the point at which the neighbourhood is to be analyzed),
   \item $x^s_j$, the geolocalised position of a neighbour of interest $j$ of the same type as point $i$,
   \item $x_j$, the geolocalised of a neighbour $j$ of $i$, whatever its type,
   \item $w(.)$, the weight of a given neighbour. In that sense, $w(x_j)$ defines the weight of a neighbor $j$ of $i$.
   \item $W_s$, the total weight of the points $x^s_j$,
   \item $W$, the total weight of all points of the dataset, whatever their type,
   \item ${\mathbf 1} \left( \left\| x^s_i - x_j \right\| \le r \right)$, the indicator function equal to 1 if $x_j$ is in the neighbourhood of $x^s_i$, e.g., the distance between $x^s_i$ and $x_j$ is at most equal to $r$, 0 otherwise.
   \item ${\mathbf 1} \left( \left\| x^s_i - x^s_j \right\| \le r \right)$, the indicator function equal to 1 if the distance between $x^s_i$ and $x^s_j$ is at most equal to $r$, 0 otherwise.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
   \item if the estimated $M$ result is above 1, the local value of the ratio is greater than the global one:,a spatial concentration of entities of type $s$ within that radius is thus detected. 
   \item if the estimated $M$ result is under 1, the local value of the ratio is lesser than the global one: a spatial dispersion of entities of type $s$ within that radius is thus detected. 
\end{itemize}   
The second remarsk concerns the significance of the results. A confidence interval can be generated thanks to Monte Carlo simulations following @Marcon2010.
...
Third, the package \textbf{dbmss} [@dbmss] on the R software [@R] can be used to compute the $M$ function.
The Euclidean distance is generally preferred for the calculation of *M* but the \textbf{dbmss} package also proposed to used network-distances.
