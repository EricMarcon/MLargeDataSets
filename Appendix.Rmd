...
The sizes are limited to 10,000 points here to save time.
To reproduce the results of the article, uncomment the full vector.

```{r}
#| label: ParamsXSizeCode
X_sizes <- c(1000, 5000, 10000)
# Uncomment to test the full range of sizes
...
test_time <- function(points_nb, FUN_DBM = X_to_M, times = 10L) {
  X <- X_csr(points_nb)
...
# RAM
library("profmem")
test_ram <- function(points_nb, FUN_DBM = X_to_M) {
  X <- X_csr(points_nb)
  profmem(FUN_DBM(X)) %>% 
    pull("bytes") %>% 
    sum(na.rm = TRUE)
...
This code chunk predicts the computation time of large point patterns.
...
The following code reproduces the figure in the article.
...
Thinning a point pattern down to a small size results in excessive variance.
The previous figure is recomputed with a 100,000-point dataset thinned down to 10,000 points.

```{r}
#| label: MthinFigSmall
# Draw the original point pattern: 100,000 points
pp_original_Matern <- X_matern(1E5)
# Repeat thinning (code from dbmss::MEnvelope)
...
Computing time of the *K~d~* , *D* and *M* functions.
...
The performances of *K~d~* and *M* are similar.
The memory required by *K~d~* decreases at 10,000 points because the approximation method by @Scholl2013 is activated.
The *D* function is around two orders of magnitude slower.
...
X_sizes %>% 
  ...
  bind_cols(
    X_sizes %>% 
    sapply(FUN = test_ram, FUN_DBM = X_to_M) %>% 
    as_tibble() %>% 
    rename(M = value)
  ) %>% 
  bind_cols(
    X_sizes %>%
    sapply(FUN = test_ram, FUN_DBM = X_to_D) %>% 
    as_tibble() %>%
    rename(D = value)
  ) %>% 
  bind_cols(
    X_sizes %>%
    sapply(FUN = test_ram, FUN_DBM = X_to_Kd) %>% 
    as_tibble() %>%
    rename(Kd = value)
  ) %>% 
  pivot_longer(cols = !Size) %>% 
  # Convert RAM to MB
  mutate(value = value / 2^20) ->
  M_ram
...
M_ram %>% 
  ggplot(aes(x = Size, y = value, color = name)) +
    ...
    scale_y_log10() + 
    labs(tag = "a") +
    labs(y = "RAM (MB)", color = "Function")
```


In sum, general conclusions can be drawn.
